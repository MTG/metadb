...
def scrape(query):
    ...
    try:
        recording = mb.get_recording_by_id(mbid, includes=["artists", "releases", "tags"])["recording"]
        ...
        for item in recording["artist-credit"]:
        ...
        for item in recording["release-list"]:

            # Request for release info
            release = mb.get_release_by_id(item["id"], includes = ["artists", "release-groups", "tags"])["release"]


            # Release group info
            release_group_id = release["release-group"]["id"]
            release_group_title = release["release-group"]["title"]
            release_group_first_release_date = release["release-group"]["first-release-date"]

            release_artists = []

            for item2 in release["artist-credit"]:
            ...
            if "tag-list" in release["release-group"]:
                release_group_tags = release["release-group"]["tag-list"]
                release_group = {"id": release_group_id, "title": release_group_title, "first-release-date": release_group_first_release_date, "tag-list": release_group_tags}
            else:
                release_group = {"id": release_group_id, "title": release_group_title, "first-release-date": release_group_first_release_date}

            # Check if release group already stored - eliminate duplicates
            if release_group not in release_groups:
                release_groups.append(release_group)


            releases.append({"id": item["id"], "title": item["title"], "artist": release_artists, "date": item["date"], "tag-list": release.get("tag-list", [])})
    ...
    if "tag-list" in recording:
        recording_tags = recording["tag-list"]
